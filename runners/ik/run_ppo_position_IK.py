...
from agents.ik.ppo_agent_position_IK import PPOAgentPosition, RolloutBatch
from envs import *

# map task names to envs classes
task_to_class = {
    'ReachCubePositionCurr': ReachCubePositionCurrEnv,
}
...
def train_ppo(args):
    ...
    for update in range(1, num_updates + 1):
        ...
        # curriculum completion handled by envs.completed flag
...
def inference_ppo(args):
    env_cls = create_environment(args.task)
    # sample a new cube position every single episode
    env = env_cls(
        vis=args.vis,
        device=args.device,
    ...
    agent = PPOAgentPosition(
        input_dim=args.state_dim if hasattr(args, 'state_dim') else env.state_dim,
        action_dim=env.action_space,
        hidden_dim=args.hidden_dim,
        device=args.device,
        checkpoint_path=args.checkpoint_path,
...
def arg_parser():
    ...
    p.add_argument('-t', '--task', type=str, default='ReachCubePositionCurr', help='Task')
    ...
    p.add_argument('--total_timesteps', type=int, default=1000000,
                   help='Total envs timesteps (train only)')
