...
class ReachCubeEgoMultimodalStackedTorqueEnv:
    ...
    def simulate_audio(self, dist):
        ...
        tone = chirp(t,1000,1000,dur)/(dist**2+1e-6)
        return tone
    ...
    def _collect_spectrograms(self):
        dists = torch.norm(self.end_effector.get_pos()-self.cube.get_pos(),dim=1).cpu().numpy()
        specs = [torch.tensor(self._compute_spectrogram(self.simulate_audio(d))).float() for d in dists]
        return torch.stack(specs).to(self.device)
    ...
    def reset(self):
        ...
        base = np.array([[0.6,0.6,0.7]]) if self.episode_count==1 else self.current_cube_pos[:1]
