...
class ReachCubeEgoMultimodalStackedEnv:
    ...
    def __init__(
        ...
        ### NEW
        train_object_count: int = 2,   # 1..4: restrict eligible object kinds for sampling
    ):
        # --- core params ---
        self.device = device
    ...
    def simulate_audio(self, dist: float, sound_id: int) -> np.ndarray:
        ...
        Uses one of four sound prototypes based on sound_id (0..3).
        """
        sr, dur = 22050, 0.01
        t = np.linspace(0, dur, int(sr * dur), endpoint=False)

        # Four source sounds (same design as your audio env)
        if sound_id == 0:
            carrier = chirp(t, f0=1000, f1=1000, t1=dur, method="linear")              # steady 1 kHz tone
        elif sound_id == 1:
            carrier = chirp(t, f0=600, f1=2400, t1=dur, method="quadratic")            # 600→2400 Hz sweep
        elif sound_id == 2:
            base = chirp(t, f0=1500, f1=1500, t1=dur, method="linear")                 # 1.5 kHz w/ tremolo
            mod = 0.5 * (1.0 + np.sin(2 * np.pi * 8 * t))
            carrier = base * mod
        else:
            carrier = chirp(t, f0=200, f1=4000, t1=dur, method="linear")               # 200→4000 Hz sweep

        # distance attenuation (inverse-square law)
        tone = carrier / (dist ** 2 + 1e-6)

        # base random background “scene” noise (sum of small chirps)
        noise = sum(
        ...
        # additional configurable Gaussian noise (dB-like)
        audio_noise_level = float(self._audio_noise_level)
        additional = np.random.normal(0, audio_noise_level, tone.shape)
