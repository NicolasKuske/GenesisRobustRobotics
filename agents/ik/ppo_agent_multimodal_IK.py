# File: agents/ik/ppo_agent_multimodal_IK.py
...
from networks.ik.ppo_multimodal_IK import PPOmultimodal


class RolloutBatchMM(NamedTuple):
    states_v:  torch.Tensor  # [T+1, N, C_v, H, W]
    states_a:  torch.Tensor  # [T+1, N, C_a, F, Tspec]
...
class PPOAgentMultimodal:
    def __init__(
        ...
        self.model = PPOmultimodal(obs_shape_vision, obs_shape_audio, action_shape).to(self.device)
    ...
    def save_checkpoint(self):
        if self.checkpoint_path:
            ...
            print(f"[PPOAgentMM] Saved checkpoint to {self.checkpoint_path}")

    def load_checkpoint(self):
        ckpt = torch.load(self.checkpoint_path, map_location=self.device)
        self.model.load_state_dict(ckpt['model_state_dict'])
        print(f"[PPOAgentMM] Loaded checkpoint from {self.checkpoint_path}")

    def eval_mode(self, enabled: bool = True):
        self.model.eval() if enabled else self.model.train()

    # ---------- Acting ----------
    @torch.no_grad()
    def select_action(self, state_v: torch.Tensor, state_a: torch.Tensor):
        """TRAIN: returns action, log_prob, entropy, value"""
        logits, value = self.model(state_v.to(self.device), state_a.to(self.device))
    ...
    @torch.no_grad()
    def act(self, state_v: torch.Tensor, state_a: torch.Tensor, deterministic: bool = False):
        """INFERENCE: action only"""
        logits, _ = self.model(state_v.to(self.device), state_a.to(self.device))
    ...
    def _compute_gae(self, rewards, values, dones, next_value):
        """
        rewards: [T, N]
        values:  [T+1, N] (already includes bootstrap at last row)
        dones:   [T, N]
        """
        T, N = rewards.shape
    ...
    def train(self, batch: RolloutBatchMM):
        with torch.no_grad():
        ...
        sv = batch.states_v[:-1].reshape(T * N, *batch.states_v.size()[2:]).to(self.device)
        sa = batch.states_a[:-1].reshape(T * N, *batch.states_a.size()[2:]).to(self.device)
        ...
        for _ in range(self.epochs):
            ...
            for start in range(0, T * N, self.batch_size):
                ...
                logits, values = self.model(sv[mb], sa[mb])
                dist = Categorical(logits=logits)
