...
class PPOAgentMultimodal:
    ...
    def train(self, states_v, states_a, actions, rewards, dones):
        """
        Perform a minimal PPO update (no value head) using clipped surrogate.
        states_v, states_a: lists of tensors length T, each (N, ...)
        actions, rewards, dones: lists of tensors length T, each (N,)
        ...
        T, N = returns.shape
        actions_flat = torch.stack(actions).view(-1)
        adv_flat = advantages.view(-1)

        # stack states for model input
        sv = torch.stack(states_v).view(T*N, *states_v[0].shape[1:])
        sa = torch.stack(states_a).view(T*N, *states_a[0].shape[1:])
