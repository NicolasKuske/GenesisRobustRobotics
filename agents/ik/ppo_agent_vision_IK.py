# agents/ik/ppo_agent_vision_IK.py
...
class PPOAgentVision:
    def __init__(
        ...
        self.model = PPOvision(obs_shape, action_dim).to(self.device)
    ...
    def select_action(self, state: torch.Tensor):
        """
        TRAINING: returns action, log_prob, entropy, value
        state: [N, C, H, W]
        """
    ...
    def _compute_gae(self, rewards, values, dones, next_value):
        """
        rewards: [T, N]
        values:  [T+1, N]  (already contains the bootstrap in the last row)
        dones:   [T, N]
        """
        T, N = rewards.shape
