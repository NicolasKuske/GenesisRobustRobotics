...
class RigidGeom(RBC):
    ...
    def _compute_sd(self, query_points):
        sd, _, _, _, _ = igl.signed_distance(query_points, self._sdf_verts, self._sdf_faces)

        return sd

    def _compute_closest_verts(self, query_points):
        _, closest_faces, _, _, _ = igl.signed_distance(query_points, self._init_verts, self._init_faces)
