# run_ppo_ego_vision_stacked_torque.py
...
def train_ppo(args):
    ...
    for update in range(1, num_updates + 1):
        # reset env and move initial state to correct device for bootstrap
        state = env.reset()
        ...
        for t in range(HORIZON):
            ...
            # env.step requires CPU tensor, so move action to CPU inside step
            state, reward, done = env.step(action)
            # move next state to device
            state = state.to(agent.device)
        ...
        # bootstrap last value on correct device
        with torch.no_grad():
            _, _, last_val = agent.model(state)
        buffers['values'].append(last_val)

        # ── ADD THIS LINE TO RECORD THE FINAL STATE ──
        buffers['states'].append(state)
...
def inference_ppo(args):
    ...
    for ep in range(1, args.num_episodes + 1):
        state = env.reset()
        state = state.to(agent.device)
        done_mask = torch.zeros(args.num_envs, dtype=torch.bool, device=agent.device)
        steps = 0
        ...
        while steps < HORIZON and not done_mask.all():
            ...
            state = state.to(agent.device)
            done_mask |= done.to(agent.device)
            steps += 1
...
def main():
    ...
    if args.load:
        ...
        if not os.path.isfile(args.checkpoint_path):
            sys.exit(f"[ERROR] Checkpoint not found: {self.checkpoint_path}")
    else:
